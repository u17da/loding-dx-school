...
const generateParagraphSummary = async (messages: Array<{role: 'user' | 'assistant', content: string}>): Promise<string> => {
  try {
    const apiMessages = [
      {
        ...
        会話全体から得られた情報を総合的に分析し、以下の情報を含む自然な段落を作成してください：
        - 失敗の概要（何が起きたのか）
        - いつ、どこで、誰が関わったか（もし言及されていれば）
        - どのような影響があったか（開発チームや製品への影響）
        - 根本的な原因や理由（なぜそれが起きたのか）
        - 改善方法や提案（どうすれば防げたか）
        
        【重要な要件】
        - 会話の最後のメッセージだけでなく、会話全体を通じて得られた情報を総合的に要約してください
        - 自然で読みやすい日本語の段落として、これらの情報を有機的につなげてください
        - 箇条書きではなく、流れるような文章にしてください
        - 学校の先生が理解しやすい表現を使用してください
        - 技術的な詳細と人間的な側面の両方を含めてください`
      },
      ...messages
    ];
    ...
    const response = await getOpenAIClient().chat.completions.create({
      model: 'gpt-4o',
      temperature: 0.4, // より自然で読みやすい文章のために適度な温度を設定
      ...
      if (!updatedData.tags) {
        try {
          const tagsResponse = await getOpenAIClient().chat.completions.create({
            model: 'gpt-4o',
            temperature: 0.7, // 多様なタグを生成するために温度を上げる
            messages: [
              {
                role: 'system',
                content: `あなたはDX（開発者体験）の失敗事例からタグと簡潔なタイトルを生成するアシスタントです。
                以下の情報から、関連する具体的なタグ（3〜5つ）と簡潔なタイトルを生成してください。
                
                【重要】以下の汎用的すぎるタグは避けてください：
                - 「教育」「学校」「先生」「児童生徒」など、ほぼすべての事例に当てはまる一般的な用語
                
                代わりに、以下のような具体的で意味のあるタグを使用してください：
                - 「ネットワーク障害」「認証エラー」「データ損失」「環境構築」「バージョン互換性」
                - 「API連携」「デプロイ失敗」「テスト不足」「ドキュメント不足」「コミュニケーション不足」
                - 「ツール選定」「プロセス改善」「モニタリング」「セキュリティ設定」「パフォーマンス」
                
                タグは具体的な問題や状況を表し、事例の分類に役立つものにしてください。`
              },
              {
                role: 'user',
